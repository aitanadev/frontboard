        ...
        <draggable tag="tr" class="c-dragable" draggable=".c-draggable__item" handle=".c-draggable__handler" :list="cols" @end="colsOrderUpdate">
          <template v-for="col in cols">
            <th
              ...
              v-on="colOn(col)"
            >
              <div class="c-datagrid__cell" :class="'c-datagrid__header__' + col.key">
                <div v-if="col.labelHTML" :class="{'c-draggable__handler': !col.sticky}" @mousedown.stop v-html="col.labelHTML"></div>
                <div v-else :class="{'c-draggable__handler': !col.sticky}" @mousedown.stop>
            ...
            >
              <div class="c-datagrid__cell" :class="'c-datagrid__header__' + col.key">
                ...
                <template v-else-if="col.key === '$'">
                    <div v-if="selectable" type="checkbox" class="c-checkbox" :checked="isAllSelection && !isIndeterminateSelection" :indeterminate="isIndeterminateSelection" @click="toggleSelection"></div>
                </template>
                <slot :name="'header.' + col.key">{{ col.label }}</slot>
            </div></th>
          </template>
          <template #footer>
            ...
            >
              <div class="c-datagrid__cell" :class="'c-datagrid__header__' + col.key">
                <div v-if="col.labelHTML" v-html="col.labelHTML"></div>
                <slot v-else :name="'header.' + col.key">{{ col.label }}</slot>
              </div>
            </th>
          </template>
        </draggable>
...
export default {
  ...
  computed: {
    ...
    isAllSelection() {
      return this.selection.length > 0 && this.selection.length === this.allRecords.length
    },
    isIndeterminateSelection() {
      return this.selection.length > 0 && !this.isAllSelection
    }
  },
