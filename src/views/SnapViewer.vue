<template>
  <div class="fds-c-snap-viewer">
    <div class="fds-c-snap-viewer__page" v-html="html"></div>
  </div>
</template>
...
export default Vue.component('SnapViewer', {
  ...
  created () {
    ...
    const blocks = Object.entries(snaps).map(([snapId, snapHtml]) => {
      ...
      return '<div class="fds-c-snap-viewer__block"><h1 class="fds-c-snap-viewer__title">' + snapId + '</h1><div class="fds-c-snap-viewer__snap">' + sanitized + '</div></div>'
    })
...
.fds-c-snap-viewer__block {
  border-radius: 3px;
  overflow: hidden;
}

.fds-c-snap-viewer__title {
  font-size: 12px;
  font-weight: 700;
  line-height: 16px;
  padding: 10px 20px;
